...
export const CreateGroomerReview = async (req, res) => {
  ...
  // 2) Body params
  const {appointmentId,  groomerId , rating, review: text } = req.body;
  if (!appointmentId || !groomerId || !rating) {
  ...
  try {
    // 3) Check appointment
    const appt = await GroomerAppointment.findById(appointmentId);
    if (!appt) {
    ...
    // 4) Prevent duplicate
    const existing = await GroomerReview.findOne({ appointment: appointmentId });
    if (existing) {
...
export const GetGroomerReviewsByProvider = async (req, res) => {
  ...
  try {
    ...
    // 3) Query & populate
    const groomerreviews = await GroomerReview.find(filter)
      .sort({ createdAt: -1 })               // newest-first
      .populate("user", "name email")        // reviewer info
      .populate("appointment", "date")       // (optional) when the appt happened
      .lean();

    return res.status(200).json({ groomerreviews });
  } catch (err) {
    console.error("Error in GetGroomerReviewsByProvider:", err);
