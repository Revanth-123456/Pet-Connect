...
const SitterAppointment = () => {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      try {
        const url = `http://localhost:5000/auth/sitterreviews/${providerType}/${providerId}`;
        const res = await axios.get(url);

        setSitterReviews(res.data.sitterreviews || []);
      } catch (err) {
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-md p-6 mt-5">
        ...
        <div className="mt-8 border-t border-gray-100 pt-6">
          <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            ...
            <span className="text-amber-600">({sitterreviews.length})</span>
          </h3>
